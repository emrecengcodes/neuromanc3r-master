from neuromanc3rBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from neuromanc3rBot import DRAGONS
...
async def saitama_is_admin(chat_id: int):
    status = False
    neuromanc3rBot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if neuromanc3rBot.id == user.id:
            status = True
            break
    return status
